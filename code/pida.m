...
% Step response
PIDeq = feedback(PID*Gp, 1);
PID2eq = Gr*feedback(Gp, Gy);
...
y_PID = lsim(PIDeq, u, t);
y_PID2 = lsim(PID2eq, u, t);
...
subplot(2,1,1);
p=plot(tt, u, 'b', tt, y_PID, 'r');
p(1).LineWidth = 1.5; 
p(2).LineWidth = 1.5; 
axis([0, tmax/2, -20, 110]);
grid;
title('PID - Step Response', 'FontSize', 16);
xlabel('Time (s)', 'FontSize', 14);
ylabel('\theta_{2} (deg)', 'FontSize', 14)
legend({'r(t)', '\theta_2(t)'}, 'FontSize', 12, 'Location', 'ne');

set(gca, 'FontName', 'Times New Roman');

subplot(2,1,2);
p=plot(tt, u, 'b', tt, y_PID2, 'r');
p(1).LineWidth = 1.5; 
p(2).LineWidth = 1.5; 
axis([0, tmax/2, -20, 110]);
xlabel('Time (s)', 'FontSize', 14);
...
%print -dpdf -painters simulacion